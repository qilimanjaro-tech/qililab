...
- Minor modification at database `Measurements`, as it now requires the `base_path` and the `relative_path` (both combined show the `result_path`). There are not changes in the `Database_Manager` inputs as this is processed internally by `StreamArray` and the `Experiment` class. The change allows for easier processing of `Database_Manager` data as showcased in qilitools' "get_path".
[#949](https://github.com/qilimanjaro-tech/qililab/pull/949)
