...
The `RSWU-SP16-TR` driver also caches the active channel: a request to change to the channel that is already set is ignored, so the instrument is not called unnecessarily.