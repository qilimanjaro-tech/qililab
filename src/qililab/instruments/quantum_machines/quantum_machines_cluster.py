...
class QuantumMachinesCluster(Instrument):
    ...
    @property
    def _config(self) -> DictQuaConfig:
        """Get the QUA config dictionary."""
        return self.settings.to_qua_config()
    ...
    def append_configuration(self, configuration: dict):
        ...
        if self._config != merged_configuration:
            # TODO: We need to convert this info into the settings! (Example: reverse `to_qua_config()` function)
            # OR maybe we don't need to store this, since its just the compiler output. And we can keep it like this :)
            temporarly_config = cast(DictQuaConfig, merged_configuration)

            # If we are already connected, reopen the connection with the new configuration
            if self._is_connected_to_qm:
                self._qm = self._qmm.open_qm(config=temporarly_config, close_other_machines=True)
                self._compiled_program_cache = {}
