...
class QuantumMachinesCluster(Instrument):
    ...
    def get_controller_type_from_bus(self, bus: str) -> str | None:
        ...
        if "RF_inputs" in self.config["elements"][bus]:
            octave = self.config["elements"][bus]["RF_inputs"]["port"][0]
            controller_name = self.config["octaves"][octave]["connectivity"]
    ...
    def get_parameter_of_bus(self, bus: str, parameter: Parameter) -> float | str | bool | tuple:
        ...
        if parameter == Parameter.IS_AMPLIFIED:
            ...
            return (
                settings_config_dict["controllers"][controller]["fems"][fem]["analog_outputs"][con_port]["output_mode"]
                == "amplified"
            )
