...
class QuantumMachinesManager(Instrument):
    ...
    def turn_on(self):
        ...
        self.qm = self.qmm.open_qm(config=self.config, close_other_machines=True)
    ...
    def update_configuration(self, compilation_config: dict):
        ...
        runcard_config = self.settings.to_qua_config()
        self.config = merge_dictionaries(runcard_config, compilation_config)
        if hasattr(self, "qm"):
            self.qm = self.qmm.open_qm(config=self.config, close_other_machines=True)
    ...
    def run_octave_calibration(self):
        ...
        for element in elements:
            self.qm.calibrate_element(element)
