...
from qililab.typings import Parameter, Pulsar, QcmQrm
...
class QbloxModule(AWG):
    ...
    def __init__(self, settings: dict):
        sequencers = settings.get("sequencers", [])
        self.sequencers = [Sequencer(**sequencer) for sequencer in sequencers]
    ...
    def initial_setup(self):
        ...
        for sequencer in self.sequencers:
            ...
            for parameter, value in sequencer.parameters.items():
                self.device.sequencers[sequencer_id].set(parameter, value)
    ...
    def _compile(self, pulse_bus_schedule: PulseBusSchedule, sequencer: Sequencer) -> QpySequence:
    ...
    def _translate_pulse_bus_schedule(self, pulse_bus_schedule: PulseBusSchedule, sequencer: Sequencer):
    ...
    def _generate_waveforms(self, pulse_bus_schedule: PulseBusSchedule, sequencer: Sequencer):
    ...
    def _get_sequencer_by_id(self, id: int):
        ...
        Returns:
            Sequencer: Sequencer with the given `id`.
        """
        for sequencer in self.sequencers:
    ...
    def get_sequencers_from_chip_port_id(self, chip_port_id: int):
        ...
        """
        if seqs := [sequencer for sequencer in self.sequencers if sequencer.chip_port_id == chip_port_id]:
            return seqs
        raise IndexError(
