...
from qililab.digital.routing._exceptions import BlockingError
...
class Block:
    ...
    def add_gate(self, gate: gates.Gate):
        ...
        if not set(gate.qubits).issubset(self.qubits):
            raise BlockingError(
                f"Gate acting on qubits {gate.qubits} can't be added " + f"to block acting on qubits {self._qubits}.",
            )
    ...
    def kak_decompose(self):  # pragma: no cover
        ...
        """
        raise NotImplementedError("KAK decomposition is not available yet.")
...
class CircuitBlocks:
    ...
    def search_by_index(self, index: int):
        ...
        for block in self.block_list:
            if block.name == index:
                return block
        raise BlockingError(f"No block found with index {index}.")
    ...
    def return_last_block(self):
        ...
        if len(self.block_list) == 0:
            raise BlockingError("No blocks found in the circuit blocks.")
...
def _initial_block_decomposition(circuit: Circuit):
    ...
    while len(all_gates) > 0:
        ...
        if len(all_gates) > 0:
            ...
            second_qubit_block_gates = _gates_on_qubit(gatelist=all_gates, qubit=second_qubit)
