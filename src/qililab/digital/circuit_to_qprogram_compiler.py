        ...
        with qp.for_loop(bin, start=0, stop=nshots - 1):
            for gate in circuit.gates:
                if isinstance(gate, Rmw):
                    gate_events = self._settings.get_gate(name="DRAG", qubits=gate.target_qubits[0])
                    ...
                    gate_events = self._settings.get_gate(name="CZ", qubits=(gate.control_qubits[0], gate.target_qubits[0]))
                    qp.sync([gate_event.bus for gate_event in gate_events])
                    for gate_event in gate_events:
                        qp.play(bus=gate_event.bus, waveform=gate_event.waveform)
                elif isinstance(gate, M):
                    for qubit in gate.qubits:
                        gate_events = self._settings.get_gate(name="M", qubits=qubit)
                        for gate_event in gate_events:
