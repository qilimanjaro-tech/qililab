...
class FiniteInputResponseB(PulseDistortion):
    """Bias tee distortion."""

    name = PulseDistortionName.FIR_B
    b: list[float]
    ...
    def apply(self, envelope: np.ndarray) -> np.ndarray:
        ...
        corr_envelope = signal.lfilter(b=self.b, a=[1], x=envelope)
    ...
    @classmethod
    def from_dict(cls, dictionary: dict) -> "FiniteInputResponseB":
