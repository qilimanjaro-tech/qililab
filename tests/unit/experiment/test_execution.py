...
def fixture_execution():
    ...
    return Experiment(platform_name=DEFAULT_PLATFORM_NAME, experiment_name=DEFAULT_EXPERIMENT_NAME).execution
...
class TestExecution:
    ...
    def test_setup_method(self, execution: Execution):
        """Test setup method."""
        connect_instruments(execution=execution)  # pylint: disable=no-value-for-parameter
        execution.setup()
        execution.close()
        ...
        assert i_0.repetition_duration == i_1.repetition_duration == execution.settings.repetition_duration

    def test_connect_method_raises_error_when_already_connected(self, execution: Execution):
