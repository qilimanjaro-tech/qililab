...
class TestBusExecution:
    ...
    def test_add_pulse_method(self, pulse_scheduled_bus: BusExecution, pulse_bus_schedule: PulseBusSchedule):
        """Test add_pulse method."""
        pulse_scheduled_bus.add_pulse_bus_schedule(pulse_bus_schedule=pulse_bus_schedule)

    def test_waveforms_method(self, pulse_scheduled_bus: BusExecution):
        """Test waveforms method."""
        for resolution in [0.01, 0.1, 1, 10]:
            pulse_scheduled_bus.waveforms(resolution=resolution)

    def test_waveforms_method_raises_error(self, pulse_scheduled_bus: BusExecution):
        """Test waveforms method raises error."""
        with pytest.raises(IndexError):
            pulse_scheduled_bus.waveforms(idx=10)

    def test_qubit_ids_property(self, pulse_scheduled_bus: BusExecution):
    ...
    def test_acquire_time_raises_error(self, pulse_scheduled_bus: BusExecution):
    ...
    def test_acquire_results_raises_error(self, pulse_scheduled_bus: BusExecution):
