...
class TestExecutionBuilder:
    ...
    def test_build_method(self, sauron_platform: Platform, pulse_schedule: PulseSchedule):
        """Test build method."""
        EXECUTION_BUILDER.build(platform=sauron_platform, pulse_schedules=[pulse_schedule])

    def test_build_method_with_wrong_pulse_bus_schedule(
        self, sauron_platform: Platform, pulse_schedule: PulseSchedule, pulse: Pulse
    ):
