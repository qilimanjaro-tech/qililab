...
from qililab.pulse.pulse_shape import FlatTop, PulseShape
...
class TestPulseShape:
    ...
    def test_envelope_methods(self, pulse_shape: FlatTop, env_params: dict[str, int]):
    ...
    def test_max_min_of_envelope_method(self, pulse_shape: FlatTop, env_params: dict[str, int]):
    ...
    def test_envelope_method_shapes(self, pulse_shape: FlatTop, env_params: dict[str, int]):
    ...
    def test_from_dict(self, pulse_shape: FlatTop):
        """Test for the to_dict method."""
        dictionary = pulse_shape.to_dict()
        pulse_shape2: FlatTop = FlatTop.from_dict(dictionary)

        dictionary2 = pulse_shape2.to_dict()
        pulse_shape3: FlatTop = FlatTop.from_dict(dictionary2)

        for shape in [pulse_shape2, pulse_shape3]:
            assert shape is not None
            assert isinstance(shape, PulseShape)
            assert isinstance(shape, FlatTop)

        assert pulse_shape == pulse_shape2 == pulse_shape3

    def test_to_dict_method(self, pulse_shape: FlatTop):
        """Test to_dict method"""
        dictionary = pulse_shape.to_dict()
