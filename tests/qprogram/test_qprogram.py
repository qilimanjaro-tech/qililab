...
class TestQProgram:
    ...
    def test_measure_error_int_length_and_weight_defined(self):
        """Test that measure raises an error if weights and integration length are given at
        the same time"""
        qp = QProgram()
        ...
        with pytest.raises(ValueError, match="Weights and integration length cannot be defined at the same time."):
            qp.measure(bus="mock_bus", waveform=drag_wf, weights=weight_wf, integration_length=100)
