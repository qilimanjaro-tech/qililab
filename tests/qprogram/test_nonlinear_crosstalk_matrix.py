...
class TestNonLinearCrosstalkMatrix:
    ...
    def test_set_non_linear_params_raises_when_only_beta_c_provided(self, nonlinear_crosstalk_matrix):
        with pytest.raises(ValueError, match="Both 'amplitude' and 'beta_c' must be provided together"):
            nonlinear_crosstalk_matrix.set_non_linear_params("flux_0", "flux_2", beta_c=-0.234)

    def test_set_non_linear_params_raises_when_only_amplitude_provided(self, nonlinear_crosstalk_matrix):
        with pytest.raises(ValueError, match="Both 'amplitude' and 'beta_c' must be provided together"):
            nonlinear_crosstalk_matrix.set_non_linear_params("flux_0", "flux_2", amplitude=-0.021)
    ...
    # --- junction_asymmetry_correction ---

    def test_junction_asymmetry_correction_zero_for_symmetric_squid(self):
        xtalk = NonLinearCrosstalkMatrix()
        assert xtalk.junction_asymmetry_correction(flux_x=0.25, d=0.0) == pytest.approx(0.0, abs=1e-10)

    def test_junction_asymmetry_correction_zero_at_zero_flux(self):
        xtalk = NonLinearCrosstalkMatrix()
        assert xtalk.junction_asymmetry_correction(flux_x=0.0, d=0.5) == pytest.approx(0.0, abs=1e-10)

    def test_junction_asymmetry_correction_known_value(self):
        xtalk = NonLinearCrosstalkMatrix()
        flux_x, d = 0.25, 1.0
        expected = -np.arctan(d * np.tan(np.pi * flux_x)) / (2 * np.pi)
        assert xtalk.junction_asymmetry_correction(flux_x=flux_x, d=d) == pytest.approx(expected, rel=1e-6)

    def test_junction_asymmetry_correction_array_input(self):
        xtalk = NonLinearCrosstalkMatrix()
        flux_x = np.array([0.0, 0.1, 0.25, 0.5])
        result = xtalk.junction_asymmetry_correction(flux_x=flux_x, d=0.5)
        assert isinstance(result, np.ndarray)
        assert result.shape == flux_x.shape

    def test_junction_asymmetry_correction_raises_on_nan_d(self):
        xtalk = NonLinearCrosstalkMatrix()
        with pytest.raises(ValueError, match="Junction asymetry cannot be NaN"):
            xtalk.junction_asymmetry_correction(flux_x=0.1, d=float("nan"))

    def test_junction_asymmetry_correction_antisymmetric_in_d(self):
        xtalk = NonLinearCrosstalkMatrix()
        flux_x, d = 0.3, 0.5
        pos = xtalk.junction_asymmetry_correction(flux_x=flux_x, d=d)
        neg = xtalk.junction_asymmetry_correction(flux_x=flux_x, d=-d)
        assert pos == pytest.approx(-neg, rel=1e-6)

    # --- get_non_linear_flux_terms ---

    def test_get_non_linear_flux_terms_returns_zero_for_none_params(self, linear_crosstalk_matrix):
        xtalk = NonLinearCrosstalkMatrix.from_linear(linear_crosstalk_matrix)
    ...
    def test_get_non_linear_flux_terms_nonzero_when_params_set(self, nonlinear_crosstalk_matrix, flux_dict):
    ...
    def test_get_non_linear_flux_terms_matches_manual_calculation(self, nonlinear_crosstalk_matrix, flux_dict):
        corrections = nonlinear_crosstalk_matrix.get_non_linear_flux_terms(flux_dict)
        expected_0 = nonlinear_crosstalk_matrix.sin_beta_scaled(flux=flux_dict["flux_2"], beta=-0.234, amp=-0.021)
        expected_1 = nonlinear_crosstalk_matrix.sin_beta_scaled(flux=flux_dict["flux_2"], beta=-0.253, amp=-0.021)
        assert corrections["flux_0"] == pytest.approx(float(expected_0), rel=1e-6)
        assert corrections["flux_1"] == pytest.approx(float(expected_1), rel=1e-6)

    def test_get_non_linear_flux_terms_junction_asym_applied(self, linear_crosstalk_matrix):
        xtalk = NonLinearCrosstalkMatrix.from_linear(linear_crosstalk_matrix)
        xtalk.set_non_linear_params("flux_0", "flux_2", junction_asym=0.3)
        flux = {"flux_0": 0.1, "flux_1": 0.2, "flux_2": 0.1}
        corrections = xtalk.get_non_linear_flux_terms(flux)
        expected = xtalk.junction_asymmetry_correction(flux_x=flux["flux_2"], d=0.3)
        assert corrections["flux_0"] == pytest.approx(float(expected), rel=1e-6)
        assert corrections["flux_1"] == pytest.approx(0.0)
        assert corrections["flux_2"] == pytest.approx(0.0)
