...
def mock_experiment_live_plot(metadata):
    ...
    with ExperimentResultsWriter(
        path=EXPERIMENT_RESULTS_PATH, metadata=metadata, live_plot=True, slurm_execution=False
    ):
        ...
...
def mock_experiment_live_plot_slurm(metadata):
    """Create a mock ExperimentResultsWriter structure for testing"""
    with ExperimentResultsWriter(path=EXPERIMENT_RESULTS_PATH, metadata=metadata, live_plot=True, slurm_execution=True):
        ...
    yield EXPERIMENT_RESULTS_PATH
    Path(EXPERIMENT_RESULTS_PATH).unlink()
