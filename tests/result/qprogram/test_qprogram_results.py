...
def fixture_qblox_qprogram_results():
    ...
    results.append_result(
        bus="readout", result=QbloxMeasurementResult(bus="readout", raw_measurement_data={"abc": 123}, shape=(1,))
    )
    return results
...
class TestsQProgramResult:
    ...
    def test_append_result_method_with_qblox(self, qblox_qprogram_results: QProgramResults):
        ...
        qblox_qprogram_results.append_result(
            "readout", result=QbloxMeasurementResult(bus="readout", raw_measurement_data={"def": 456}, shape=(1,))
        )
        assert len(qblox_qprogram_results.results["readout"]) == 2

        qblox_qprogram_results.append_result(
            "another_readout", result=QbloxMeasurementResult(bus="another_readout", raw_measurement_data={"def": 456}, shape=(1,))
        )
