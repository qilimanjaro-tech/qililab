...
class TestGateSettings:
    ...
    def test_attributes(self, gate_settings):
        ...
        assert all(
            ...
            for key, settings in gate_settings.gates.items()
            for event in settings
        )
        assert isinstance(gate_settings.reset_method, str)
        assert isinstance(gate_settings.passive_reset_duration, int)
        assert isinstance(gate_settings.operations, list)

    def test_get_operation_settings(self, gate_settings):
        """Test the ``get_operation_settings`` method of the GateSettings class."""
        for operation in gate_settings.operations:
            if isinstance(operation, dict):
                operation = Runcard.GateSettings.OperationSettings(**operation)
            assert isinstance(
                gate_settings.get_operation_settings(name=operation.name),
                gate_settings.OperationSettings,
            )

    def test_get_operation_settings_raises_error_when_operation_does_not_exist(self, gate_settings):
        ...
        with pytest.raises(ValueError, match=f"Operation {name} not found in gate settings."):
            gate_settings.get_operation_settings(name)

    def test_get_gate(self, gate_settings):
        """Test the ``get_gate`` method of the GateSettings class."""
        gates_qubits = [
            (re.search(GATE_ALIAS_REGEX, alias)["gate"], re.search(GATE_ALIAS_REGEX, alias)["qubits"])
            for alias in gate_settings.gates.keys()
        ]
        assert all(
            isinstance(gate_event, GateEventSettings)
            for gate_name, gate_qubits in gates_qubits
            for gate_event in gate_settings.get_gate(name=gate_name, qubits=ast.literal_eval(gate_qubits))
        )

    def test_get_gate_raises_error(self, gate_settings):
        ...
        with pytest.raises(KeyError, match=error_string):
            gate_settings.get_gate(name, qubits=qubits)

    def test_gate_names(self, gate_settings):
        """Test the ``gate_names`` method of the GateSettings class."""
        expected_names = list(gate_settings.gates.keys())
        assert gate_settings.gate_names == expected_names

    def test_set_platform_parameters(self, gate_settings):
        """Test that with ``set_parameter`` we can change all settings of the platform."""
        gate_settings.set_parameter(parameter=Parameter.DELAY_BEFORE_READOUT, value=1234)
        assert gate_settings.delay_before_readout == 1234

        gate_settings.set_parameter(parameter=Parameter.DELAY_BETWEEN_PULSES, value=1234)
        assert gate_settings.delay_between_pulses == 1234

    @pytest.mark.parametrize("alias", ["X(0)", "M(0)"])
    def test_set_gate_parameters(self, alias: str, gate_settings):
        ...
        gate_settings.set_parameter(alias=alias, parameter=Parameter.DURATION, value=1234)
        assert gate_settings.get_gate(name=name, qubits=qubits)[0].pulse.duration == 1234

        gate_settings.set_parameter(alias=alias, parameter=Parameter.PHASE, value=1234)
        assert gate_settings.get_gate(name=name, qubits=qubits)[0].pulse.phase == 1234

        gate_settings.set_parameter(alias=alias, parameter=Parameter.AMPLITUDE, value=1234)
        assert gate_settings.get_gate(name=name, qubits=qubits)[0].pulse.amplitude == 1234

    @pytest.mark.parametrize("alias", ["X(0,)", "X()", "X", ""])
    def test_set_gate_parameters_raises_error_when_alias_has_incorrect_format(self, alias: str, gate_settings):
